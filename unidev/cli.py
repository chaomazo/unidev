"""Command-line entry point for the Unidev starter toolkit."""
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="unidev", description="Bootstrap and inspect a Unidev production workspace.")
    subparsers = parser.add_subparsers(dest="command", required=True)
    init_parser = subparsers.add_parser("init", description="Create the standard Unidev workspace folders.")
...
def init_workspace(path: str | Path) -> int:
    ...
    manifest = root / "unidev.yaml"
    if not manifest.exists():
        manifest.write_text("# Unidev workspace manifest\nversion: 1\npipelines: pipeline_defs\nskills: skills\ntools: tools\nproductions: productions\n", encoding="utf-8")
    print(f"Unidev workspace ready: {root}")
    return 0
