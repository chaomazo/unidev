"""Command-line entry point for the ScenePilot starter toolkit."""
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="scenepilot", description="Bootstrap and inspect a ScenePilot production workspace.")
    subparsers = parser.add_subparsers(dest="command", required=True)
    init_parser = subparsers.add_parser("init", description="Create the standard ScenePilot workspace folders.")
...
def init_workspace(path: str | Path) -> int:
    ...
    manifest = root / "scenepilot.yaml"
    if not manifest.exists():
        manifest.write_text("# ScenePilot workspace manifest\nversion: 1\npipelines: pipeline_defs\nskills: skills\ntools: tools\nproductions: productions\n", encoding="utf-8")
    print(f"ScenePilot workspace ready: {root}")
    return 0
